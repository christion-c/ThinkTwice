...
Code or schemas intentionally shared across services live here. A change
here can affect the backend, frontend, and ML service at once — double
check all three before committing, even though one person (Christion) owns
the whole project now and there's no separate owner to coordinate with.
