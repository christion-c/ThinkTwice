/**
 * Pure math behind the Finance/Fuel screens' budget and refill forecasts.
 * Separated from FinanceContext.tsx so these formulas can be unit tested
 * and reasoned about independently of React state/persistence concerns.
 */
...
/**
 * Turns a list of samples into one representative value: takes the most
 * recent 20, discards outliers more than 3 median-absolute-deviations
 * from the median (a single wildly-off fill-up shouldn't skew the
 * forecast), then exponentially weights what's left so recent entries
...
/**
 * Combines the user's manual inputs with their fill-up history stats to
 * produce the budget/refill numbers the Finance and Fuel screens show.
 * Manual inputs and history-derived typicals are blended (weighted
 * toward history once there's enough of it) rather than one fully
 * overriding the other, so a single stale manual entry doesn't swing the
 * forecast and a brand-new account without history still gets a usable
 * estimate from whatever the user just typed in.
 */
